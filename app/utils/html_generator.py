...
class HTMLGenerator:
    ...
    def generar_reporte_invernadero(self, invernadero, resultados_simulacion=None, registro_instrucciones=None):
        ...
        html.insertar_al_final("<meta name='viewport' content='width=device-width, initial-scale=1.0'>")
        html.insertar_al_final(f"<title>Reporte - {invernadero.nombre}</title>")
        ...
        html.insertar_al_final(f"<h1>Reporte de Simulación - {invernadero.nombre}</h1>")
        html.insertar_al_final(f"<p class='fecha'>Generado el: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}</p>")
        ...
        html.insertar_al_final(f"<tr><td><strong>Nombre:</strong></td><td>{invernadero.nombre}</td></tr>")
        html.insertar_al_final(f"<tr><td><strong>Número de hileras:</strong></td><td>{invernadero.hileras.obtener_tamaño()}</td></tr>")
        html.insertar_al_final(f"<tr><td><strong>Drones asignados:</strong></td><td>{invernadero.drones.obtener_tamaño()}</td></tr>")
        html.insertar_al_final(f"<tr><td><strong>Plan de riego:</strong></td><td>{invernadero.plan_riego.obtener_plan_original()}</td></tr>")
        ...
        if resultados_simulacion:
            ...
            html.insertar_al_final(self._generar_resultados_simulacion(resultados_simulacion))
        
        # Instrucciones por tiempo
        if registro_instrucciones:
            html.insertar_al_final("<h2>Instrucciones por Tiempo</h2>")
            html.insertar_al_final(self._generar_tabla_instrucciones(registro_instrucciones))
        
        # Estadísticas finales
        if resultados_simulacion:
            html.insertar_al_final("<h2>Estadísticas Finales</h2>")
            html.insertar_al_final(self._generar_estadisticas_finales(resultados_simulacion))
    ...
    def _generar_tabla_plantas(self, invernadero):
        ...
        tabla.insertar_al_final("<thead><tr><th>Hilera</th><th>Posición</th><th>Tipo de Planta</th><th>Agua (L)</th><th>Fertilizante (g)</th></tr></thead>")
    ...
    def _generar_tabla_drones(self, invernadero):
        tabla = ListaEnlazada()
        tabla.insertar_al_final("<table>")
        tabla.insertar_al_final("<thead><tr><th>Hilera</th><th>Dron</th></tr></thead>")
        tabla.insertar_al_final("<tbody>")
        ...
        for i in range(invernadero.drones.obtener_tamaño()):
            ...
            tabla.insertar_al_final(f"<tr><td>H{hilera_num}</td><td>{dron.nombre}</td></tr>")
    ...
    def _generar_resultados_simulacion(self, resultados):
        ...
        resultado.insertar_al_final(f"<div class='stat-box'><div class='stat-number'>{resultados.tiempo_total}</div><div class='stat-label'>Segundos</div></div>")
        resultado.insertar_al_final(f"<div class='stat-box'><div class='stat-number'>{resultados.total_agua}</div><div class='stat-label'>Litros de Agua</div></div>")
        resultado.insertar_al_final(f"<div class='stat-box'><div class='stat-number'>{resultados.total_fertilizante}</div><div class='stat-label'>Gramos de Fertilizante</div></div>")
        ...
        resultado.insertar_al_final("<thead><tr><th>Dron</th><th>Agua Utilizada (L)</th><th>Fertilizante Utilizado (g)</th></tr></thead>")
        ...
        for i in range(resultados.resultados_drones.obtener_tamaño()):
            ...
            resultado.insertar_al_final(f"<tr><td>{resultado_dron.id}</td><td>{resultado_dron.agua_utilizada}</td><td>{resultado_dron.fertilizante_utilizado}</td></tr>")
    ...
    def _generar_estadisticas_finales(self, resultados):
        ...
        estadisticas.insertar_al_final("<p><strong>Información del plan de riego y aplicación de fertilizante robotizado:</strong></p>")
        estadisticas.insertar_al_final("<ul>")
        estadisticas.insertar_al_final(f"<li><strong>Tiempo para regado óptimo:</strong> {resultados.tiempo_total} segundos</li>")
        estadisticas.insertar_al_final("<li><strong>Agua requerida por dron:</strong><ul>")
        
        for i in range(resultados.resultados_drones.obtener_tamaño()):
            resultado_dron = resultados.resultados_drones.obtener(i)
            estadisticas.insertar_al_final(f"<li>{resultado_dron.id} - {resultado_dron.agua_utilizada} litros</li>")
        
        estadisticas.insertar_al_final(f"<li><strong>TOTAL:</strong> {resultados.total_agua} litros</li>")
        estadisticas.insertar_al_final("</ul></li>")
        estadisticas.insertar_al_final("<li><strong>Fertilizante requerido por dron:</strong><ul>")
        
        for i in range(resultados.resultados_drones.obtener_tamaño()):
            resultado_dron = resultados.resultados_drones.obtener(i)
            estadisticas.insertar_al_final(f"<li>{resultado_dron.id} - {resultado_dron.fertilizante_utilizado} gramos</li>")
        
        estadisticas.insertar_al_final(f"<li><strong>TOTAL:</strong> {resultados.total_fertilizante} gramos</li>")
        estadisticas.insertar_al_final("</ul></li>")
        estadisticas.insertar_al_final("</ul>")
